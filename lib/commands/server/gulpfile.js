...
gulp.task('weex:js', (done) => {
    var tasks = [];
    getFiles.getAllFiles(path.resolve(process.cwd(), 'native_dist/'), 'js').map((element) => {
        var distDir = path.resolve(element);
        tasks.push(gulp.src(element)
            .pipe(weexUtil.addFramework())
            );
    })
    ...
    return merge(tasks).pipe(logger.time('build-weex-js')).on('end', (done) => {
        // 修复只能打pages的bug
        var _source = path.resolve(process.cwd(), 'native_dist/js'),
            _tmppath = path.resolve(process.cwd(), 'native_dist/_pages');

        shell.mkdir('-p', _tmppath);
        shell.cp('-r', _source, _tmppath);
        fs.rename(path.resolve(_tmppath, 'js'), path.resolve(_tmppath, 'pages'), function(err){
            if (!err) return;
            console.log("Error renaming file:", err );
        });
        gulp.src('src/assets/fonts/iconfont.ttf')
            .pipe(logger.time('build-iconfont'))
            .pipe(weexUtil.getIconfontMd5())
            .pipe(gulp.dest('native_dist/_pages/iconfont', {
                mode: '0777'
            }))
            .on('end', function() {
                gulp.src('src/assets/images/**/*')
                    .pipe(weexUtil.getAssetsMd5())
                    .pipe(logger.time('build-images'))
                    .pipe(gulp.dest('native_dist/_pages/images', {
                        mode: '0777'
                    }))
                    .on('end', function() {
...
gulp.task('handle-config-native', (done) => {
    ...
    weexErosPackUtil._writeJsonToIos('btr.native.json', nativeConfig)
    weexErosPackUtil._writeJsonToAndroid('btr.native.json', nativeConfig)
    if (isFirstReload) {
        isFirstReload = false
        logger.log('Reload btr.native.js done.')
    } else {
        logger.success('Reload btr.native.js done, you can rebuild your app to take effect.')
    }
    done()
})
...
gulp.task('watch', (done) => {
    ...
    gulp.watch('config/btr.native.js', ['handle-config-native'])
...
gulp.task('weex-eros:ios', (callback) => {
    gulpSequence('weex-eros-ios', 'handle-config-native', 'weex:js')(callback)
});
